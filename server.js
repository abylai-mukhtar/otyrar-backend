...
async function ensureStore() {
  ...
  try {
    ...
    await fs.writeFile(DATA_FILE, JSON.stringify(makeDefaultStore(), null, 2), "utf8");
  }
}
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (user !== store.admin.user || pass !== store.admin.pass) {
...
app.patch("/api/admin/password", requireAuth, async (req, res) => {
  ...
  if (!oldPass || !newPass || String(newPass).length < 6) {
...
app.get("/api/public/content", async (req, res) => {
  const store = await readStore();
  res.json({ content: store.content, updatedAt: store.updatedAt });
});

app.use(express.static(ROOT_DIR));

app.listen(PORT, () => {
  console.log(`Otyrar CMS backend running on http://localhost:${PORT}`);
});
